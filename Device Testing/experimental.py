...
def check_spike_connected():
    # Compare values and check for spiking
    if len(z_arr_18) > 15:
        diffx = x_arr_18[-1] - x_arr_19[-1]
        diffy = y_arr_18[-1] - y_arr_19[-1]
        diffz = z_arr_18[-1] - z_arr_19[-1]
        global disconnected_spike
        disconnected_spike = 0
        if(disconnected_spike == 1):
            print(' OFFLINE SPIKE!!!!')
            disconnected_spike = 0

        if (abs(diffx) > abs(diffxth) + 0.8 or abs(diffx) < abs(diffxth) - 0.8):
            print('your drink may have been spiked')
        if (abs(diffy) > abs(diffyth) + 0.8 or abs(diffy) < abs(diffyth) - 0.8):
            print('your drink may have been spiked')
        if (abs(diffz) > abs(diffzth) + 0.6 or abs(diffz) < abs(diffzth) - 0.6):
            print('your drink may have been spiked')
    # Take stable threshold values once values stabilise (16th value)
    else:
        diffxth = x_arr_18[-1] - x_arr_19[-1]
        diffyth = y_arr_18[-1] - y_arr_19[-1]
        diffzth = z_arr_18[-1] - z_arr_19[-1]

def check_spike_disconnected():
    # Compare values and check for spiking
    if len(z_arr_18) > 15:
        diffx = x_arr_18[-1] - x_arr_19[-1]
        diffy = y_arr_18[-1] - y_arr_19[-1]
...
while True:
    ...
    print(f"threshold X  - {diffxth}")
    print(f"threshold Y  - {diffyth}")
    print(f"threshold Z  - {diffzth}\n")
